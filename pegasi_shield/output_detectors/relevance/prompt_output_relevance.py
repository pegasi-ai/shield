...
class PromptOutputRelevance(BaseRelevanceDetector):
    def scan(self, prompt: str, output: str, context: str) -> (str, bool, float):
        if output.strip() == "":
            return "Prompt & Output", True, 0.0

        # Calculate semantic similarity
        similarity = self._calculate_similarity(prompt, output)
        ...
        mismatch_score = self._check_entity_mismatch(prompt, output)
        ...
        return "Prompt & Output", is_relevant, risk_score
